...
export class ApplicationsubmitComponent {
  ...
  submitApplication(propertyId: string): void {
    ...
    localDate.setMinutes(localDate.getMinutes() - offset); // Adjust for the timezone offset

    // Format the adjusted date as an ISO string
    const adjustedDateTime = localDate.toISOString();
    const application = {
      id:id,
      propertyId:propertyId,  
      tenantType: this.applicationData.tenantType,
      comments: this.applicationData.comments,
      submittedAt: adjustedDateTime,
      status: 'Pending'
    };
